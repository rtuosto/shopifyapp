...
- **Storefront Overlay Preview System**: Enables merchants to preview AI recommendations directly on their actual product pages with an interactive overlay UI. Uses opaque tokens (15-min TTL) and postMessage communication. Supports toggling between control/variant views before approval. Sets foundation for future WYSIWYG editing and "Copilot for Shopify" vision. Public SDK endpoints use CORS for cross-origin storefront access. **SDK Installation Guidance**: Settings page provides clear installation instructions with copy-paste script tag. Warning banners on AI Recommendations page alert users when SDK is not installed, explaining why preview links show blank pages. Preview error handling displays helpful setup instructions instead of silent failures.
